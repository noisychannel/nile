...
def analysis(all_res, total_iterations):
    mdata = []
    for exp_id, exp_res in enumerate(all_res):
        if len(exp_res) != 0:
            ...
            mdata.append([exp_id+1, best_run, exp_res[best_run][0], it_string, exp_res[best_run][1], exp_res[best_run][2], '-', exp_res[best_run][3], round(np.mean(dev_res), 4), round(np.std(dev_res), 4)])
        else:
...
for exp in range(1, 17):
    ...
    for run in range(1, 11):
        if exp < 9:
            err_file = log_dir + "/medium." + str(exp) + "." + str(run) + ".err"
            baseline_dev = 0.246188852765
        else:
            err_file = log_dir + "/full." + str(exp) + "." + str(run) + ".err"
    ...
    total_iterations.append((np.max(exp_total_iterations), np.min(exp_total_iterations)))
